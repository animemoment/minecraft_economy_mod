...
public class EconomyTradeMenu extends AbstractContainerMenu {
    ...
    // Константы слотов
    private static final int OWNER_SLOTS = 36;
    ...
    private static final int BUY_START = 72;
    private static final int BUY_END = 80;
    private static final int SELL_START = 81;
    private static final int SELL_END = 89;
    ...
    public EconomyTradeMenu(int id, Inventory playerInv, IEconomicActor owner) {
        ...
        // Принудительная инициализация инвентаря жителя
    ...
    public void updateFromServer(List<ItemStack> items, long budget) {
        for (int i = 0; i < Math.min(items.size(), OWNER_SLOTS); i++) ownerInventory.setItem(i, items.get(i));
        this.clientBudget = budget;
    }
    ...
    @Override
    public boolean stillValid(Player p) { return ownerActor != null; }
